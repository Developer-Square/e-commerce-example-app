...
import type { Message } from './email.types';

export const transport = nodemailer.createTransport(envVariables.email.smtp);

if (envVariables.env !== 'test') {
  transport
    .verify()
...
export const sendResetPasswordEmail = async (
  ...
  const resetPasswordUrl = `http://${envVariables.siteUrl}/reset-password?token=${token}`;
  const text = `Hi,
...
export const sendVerificationEmail = async (
  ...
  const verificationEmailUrl = `http://${envVariables.siteUrl}/verify-email?token=${token}`;
  const text = `Hi ${name},
...
export const sendAccountCreated = async (
  ...
  // replace this url with the link to the email verification page of your front-end app
  const loginUrl = `http://${envVariables.siteUrl}/signin`;
  const text = `Hi ${name},
