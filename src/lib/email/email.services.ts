...
export const sendResetPasswordEmail = async (
  ...
  const resetPasswordUrl = `${envVariables.siteUrl}/reset-password?token=${token}`;
  const text = `Hi,
...
export const sendVerificationEmail = async (
  ...
  const verificationEmailUrl = `${envVariables.siteUrl}/verify-email?token=${token}`;
  const text = `Hi ${name},
...
export const sendAccountCreated = async (
  ...
  // replace this url with the link to the email verification page of your front-end app
  const loginUrl = `${envVariables.siteUrl}/signin`;
  const text = `Hi ${name},
